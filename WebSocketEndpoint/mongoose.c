...
void mg_mqtt_pub(struct mg_connection *c, const struct mg_mqtt_opts *opts) {
  ...
  //MG_DEBUG(("%lu [%.*s] -> [%.*s]", c->id, (int) opts->topic.len, (char *) opts->topic.ptr, (int) opts->message.len, (char *) opts->message.ptr));
  if (opts->qos > 0) len += 2;
      ...
      if (rc == MQTT_MALFORMED) {
        ...
        switch (mm.cmd) {
          case MQTT_CMD_CONNACK:
            ...
            break;
          case MQTT_CMD_PUBLISH: {
            //MG_DEBUG(("%lu [%.*s] -> [%.*s]", c->id, (int) mm.topic.len, mm.topic.ptr, (int) mm.data.len, mm.data.ptr));
            if (mm.qos > 0) {
...
bool mg_send(struct mg_connection *c, const void *buf, size_t len) {
  if (c->is_udp) {
    ...
    //MG_DEBUG(("%lu %p %d:%d %ld err %d", c->id, c->fd, (int) c->send.len, (int) c->recv.len, n, MG_SOCK_ERR(n)));
...
static void read_conn(struct mg_connection *c) {
  ...
  if (c->recv.len >= MG_MAX_RECV_SIZE) {
    ...
    //MG_DEBUG(("%lu %p snd %ld/%ld rcv %ld/%ld n=%ld err=%d", c->id, c->fd, (long) c->send.len, (long) c->send.size, (long) c->recv.len, (long) c->recv.size, n, MG_SOCK_ERR(n)));
    iolog(c, buf, n, true);
  }
}
...
static void accept_conn(struct mg_mgr *mgr, struct mg_connection *lsn) {
  ...
  if (fd == MG_INVALID_SOCKET) {
    ...
    //MG_DEBUG(("%lu %p accepted %M -> %M", c->id, c->fd, mg_print_ip_port, &c->rem, mg_print_ip_port, &c->loc));
    mg_call(c, MG_EV_OPEN, NULL);
...
void mg_tls_init(struct mg_connection *c, const struct mg_tls_opts *opts) {
  ...
  //MG_DEBUG(("%lu Setting TLS, CA: %s, cert: %s, key: %s", c->id, opts->ca == NULL ? "null" : opts->ca, opts->cert == NULL ? "null" : opts->cert, opts->certkey == NULL ? "null" : opts->certkey));
...
static bool mg_tcpip_driver_stm32_up(struct mg_tcpip_if *ifp) {
  ...
  if ((ifp->state == MG_TCPIP_STATE_DOWN) && up) {  // link state just went up
    ...
    //MG_DEBUG(("Link is %uM %s-duplex", maccr & BIT(14) ? 100 : 10, maccr & BIT(11) ? "full" : "half"));
  }
  return up;
}
...
static bool mg_tcpip_driver_stm32h_up(struct mg_tcpip_if *ifp) {
  ...
  if ((ifp->state == MG_TCPIP_STATE_DOWN) && up) {  // link state just went up
    ...
    ETH->MACCR = maccr;  // IRQ handler does not fiddle with this register
    //MG_DEBUG(("Link is %uM %s-duplex", maccr & BIT(14) ? 100 : 10, maccr & BIT(13) ? "full" : "half"));
  }
  return up;
}
...
static bool mg_tcpip_driver_tm4c_up(struct mg_tcpip_if *ifp) {
  ...
  if ((ifp->state == MG_TCPIP_STATE_DOWN) && up) {  // link state just went up
    ...
    //MG_DEBUG(("Link is %uM %s-duplex", emaccfg & BIT(14) ? 100 : 10, emaccfg & BIT(11) ? "full" : "half"));
  }
  return up;
}
...
static void rx_arp(struct mg_tcpip_if *ifp, struct pkt *pkt) {
  if (pkt->arp->op == mg_htons(1) && pkt->arp->tpa == ifp->ip) {
    ...
    *arp = *pkt->arp;
    ...
    arp->tpa = pkt->arp->spa;
    arp->spa = ifp->ip;
    //MG_DEBUG(("ARP: tell %M we're %M", mg_print_ip4, &arp->tpa, mg_print_ip4, &ifp->ip));
    ether_output(ifp, PDIFF(eth, arp + 1));
...
static size_t tx_tcp(struct mg_tcpip_if *ifp, uint8_t *dst_mac, uint32_t dst_ip,
  ...
  tcp->csum = csumfin(cs);
  //MG_DEBUG(("TCP %M:%hu -> %M:%hu fl %x len %u", mg_print_ip4, &ip->src, mg_ntohs(tcp->sport), mg_print_ip4, &ip->dst, mg_ntohs(tcp->dport), tcp->flags, (int) len));
...
#if 0
    //MG_DEBUG(("%lu %d %M:%hu -> %M:%hu", c->id, (int) pkt->raw.len, mg_print_ip4, &pkt->ip->src, mg_ntohs(pkt->tcp->sport), mg_print_ip4, &pkt->ip->dst, mg_ntohs(pkt->tcp->dport)));
...
static void rx_ip(struct mg_tcpip_if *ifp, struct pkt *pkt) {
  if (pkt->ip->proto == 1) {
    ...
    mkpay(pkt, pkt->udp + 1);
    //MG_DEBUG(("UDP %M:%hu -> %M:%hu len %u", mg_print_ip4, &pkt->ip->src, mg_ntohs(pkt->udp->sport), mg_print_ip4, &pkt->ip->dst, mg_ntohs(pkt->udp->dport), (int) pkt->pay.len));
    ...
    //MG_DEBUG(("TCP %M:%hu -> %M:%hu len %u", mg_print_ip4, &pkt->ip->src, mg_ntohs(pkt->tcp->sport), mg_print_ip4, &pkt->ip->dst, mg_ntohs(pkt->tcp->dport), (int) pkt->pay.len));
    rx_tcp(ifp, pkt);
  }
}
